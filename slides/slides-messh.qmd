...
# 3 — Le RAG : récupérer, ancrer, citer

## Cirdi : un RAG sur la collection HAL du CIRED

\begin{center}
\includegraphics[width=0.9\linewidth]{slides-assets/hal-header.png}
\end{center}

\small Le corpus vit dans HAL. Cirdi y ajoute un accès en trois temps :

1. **Récupération** --- embeddings → passages pertinents
2. **Génération ancrée** --- réponse fondée sur ces passages
3. **Citation** --- sources HAL tracées

\smallskip
**Open source**, réplicable sur toute collection HAL.
...
## Du PDF au passage indexable

- **Corpus** : publications CIRED déposées dans HAL --- texte intégral préféré au PDF, enrichi des métadonnées
- **Découpage** : passages indexés par embeddings (le choix de granularité pèse sur la qualité)
- **Hébergement modeste** : serveur dédié, `mistral-small` dominant, Ollama via Huma-Num
- **Journaux d'usage** anonymisés dès la collecte (RGPD), rapport d'analyse **Quarto reproductible**

::: {.notes}
Le retour d'expérience honnête : les choix de corpus et de découpage sont des
choix de méthode, pas des détails techniques. C'est ce que ce public veut entendre.
:::

# 5 — La synthèse agrège et trace ses sources

## Ce que la synthèse apporte --- et où elle cite
...
Le type de requête **dominant** : la **synthèse thématique**, loin devant la recherche de publication.

\medskip
La synthèse :

- **agrège** plusieurs papiers
- **reformule** dans les termes de la question
- **trace** chaque affirmation vers sa source
::::
:::::
