...
function handleResponse(requestBody, data, queryId) {
    ...
    // Prepare header with query and date
    const today = new Date();
    const dayDate = today.toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    const headerHtml = `<h2>${escapeHtml(requestBody.query)}</h2><div><em>Generated by Cirdi on ${dayDate}</em></div><hr/>`;

    const htmlContent = headerHtml + renderSafeLLMContent(processedContent);
